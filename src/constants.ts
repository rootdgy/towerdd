...
// FIXED: Clean "S" shape path for Glacier to prevent confusing movement
const PATH_GLACIER: Coordinate[] = [];
// Segment 1: Left to Right (Top)
for(let x=0; x<=18; x++) PATH_GLACIER.push({x, y: 1});
// Segment 2: Down
for(let y=1; y<=5; y++) PATH_GLACIER.push({x: 18, y});
// Segment 3: Right to Left (Middle)
for(let x=18; x>=1; x--) PATH_GLACIER.push({x, y: 5});
// Segment 4: Down
for(let y=5; y<=9; y++) PATH_GLACIER.push({x: 1, y});
// Segment 5: Left to Right (Bottom) to Base
for(let x=1; x<=19; x++) PATH_GLACIER.push({x, y: 9});

const PATH_GLACIER_VAR_1: Coordinate[] = PATH_GLACIER.map(p => ({ x: p.x, y: GRID_H - 1 - p.y }));

// FIXED: Clean Spiral path for Volcano
const PATH_VOLCANO: Coordinate[] = [];
// Outer Ring
for(let x=0; x<=19; x++) PATH_VOLCANO.push({x, y: 0});
for(let y=0; y<=11; y++) PATH_VOLCANO.push({x: 19, y});
for(let x=19; x>=0; x--) PATH_VOLCANO.push({x, y: 11});
for(let y=11; y>=2; y--) PATH_VOLCANO.push({x: 0, y});
// Middle Ring
for(let x=0; x<=17; x++) PATH_VOLCANO.push({x, y: 2});
for(let y=2; y<=9; y++) PATH_VOLCANO.push({x: 17, y});
for(let x=17; x>=2; x--) PATH_VOLCANO.push({x, y: 9});
for(let y=9; y>=4; y--) PATH_VOLCANO.push({x: 2, y});
// Inner Path to Base
for(let x=2; x<=15; x++) PATH_VOLCANO.push({x, y: 4});
